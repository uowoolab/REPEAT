...
/*          my_complex eigenvalue, the corresponding my_complex eigenvector */
...
/*          A my_complex eigenvector corresponding to a my_complex eigenvalue */
...
/*          column and each selected my_complex eigenvector occupies two */
/*          columns. */
...
/*          > 0:  the 2-by-2 block (INFO:INFO+1) does not have a my_complex */
/*                eigenvalue. */
...
/* L20: */
	...
	*info = -7;
    } else if (my_compl && *ldvl < *n || *ldvl < 1) {
	*info = -10;
...
/*           (b) this would be the second of a my_complex pair. */
/*           Check for my_complex eigenvalue, so as to be sure of which */
...
/*           (b) this would be the second of a my_complex pair. */
/*           Check for my_complex eigenvalue, so as to be sure of which */
/*           entry(-ies) of SELECT to look at -- if my_complex, SELECT(JE) */
/*           or SELECT(JE-1). */
/*           If this is a my_complex pair, the 2-by-2 diagonal block */
